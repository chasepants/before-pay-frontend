...
const CreateSavingsGoal = () => {
  ...
  const handleManualSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post('http://localhost:3001/api/savings-goal', newGoal, { withCredentials: true });
  ...
  const addToSavings = async (product) => {
    ...
    try {
      const res = await axios.post('http://localhost:3001/api/savings-goal', { userId: user._id, ...product, targetAmount: product.extracted_price }, { withCredentials: true });
      dispatch(addSavingsGoal(res.data));
    } catch (err) {
